...
def plot_graph_k_nm(k,n,m,G,list_of_Graphs_final, Gagr,uuu,d1=0,d2=2,d3=0,d4=.3,d5=1,d6=.4,d7=.2,d8=1,d9=0.3,colors_grey='gray',nodesize=1000,withlabels=True,edgelist=[],layout=True,b_alpha=0.5):  
    '''
    ...
    fig=plt.figure(num=uuu,figsize=(24,20))
    ...
    # # print G.nodes()
    # nx.draw_networkx_edges(G,created_pos,edgelist=edgelist,edge_color='k',alpha=0.2)
    plt.title('Multilayer Temporal 2-Mode Network',{'size': '20'})
    plt.axis('off')
    # plt.show()
...
def plot_graph_k_n(k,n,G,list_of_Graphs_final, Gagr,d1=0,d2=2,d3=0,d4=.3,d5=1,d6=.4,d7=.2,d8=1,d9=0.3,colors_grey='gray',nodesize=1000,withlabels=True,edgelist=[],layout=True,b_alpha=0.5):  
    ...
    for i in range(k):
        ...
        ya.append(xylevels[i][0][1])
        plt.plot(xa,ya,'-',color=levels[i][2])
        for ndd in list_of_Graphs_final[i].nodes(data=True):
            labels[ndd[0]]=ndd[1]['label']
        # print list_of_Graphs_final[i].nodes(data=True)
        nx.draw(list_of_Graphs_final[i],created_pos,with_labels=withlabels,nodelist=list(levels[i][0]),node_color=levels[i][2],node_size=nodesize,edge_color=levels[i][2],alpha=0.7)
        nx.draw_networkx_labels(list_of_Graphs_final[i],created_pos,labels)


        # nx.draw(list_of_Graphs_final[i],created_pos,with_labels=withlabels,nodelist=list(levels[i][0]),node_color=levels[i][2],node_size=nodesize,edge_color=levels[i][2],alpha=0.7)
    # print edgelist
    # print G.nodes()
    nx.draw_networkx_edges(G,created_pos,edgelist=edgelist,edge_color='k',alpha=0.6)
    # plt.title('Multilayer Temporal Network',{'size': '20'})
    # plt.show()
...
# print G.nodes()
# 
def synthetic_multi_level_dict(dict_of_graphs,pernode_dict,nodper_dic,ract_dic,No_isolates=True):

    list_of_Graphs=[]
    number_of_nodes=0
    for kk in sorted(ract_dic.keys()):
        ggr=dict_of_graphs[ract_dic[kk]]
        if len(ggr.nodes())>number_of_nodes:
            number_of_nodes=len(ggr.nodes())
        list_of_Graphs.append(ggr)
        n=number_of_nodes
    list_of_isolates=[]
    list_of_Graphs_final=[]
    k=len(list_of_Graphs)
    for ij in range(k):
    #     list_of_Graphs.append(nx.erdos_renyi_graph(n,p[ij]))
        list_of_isolates.append(nx.isolates(list_of_Graphs[ij]))
        for nd in nodper_dic:
            if list_of_Graphs[ij].has_node(nd):
                continue
            else:
                list_of_Graphs[ij].add_node(nd,label=nodper_dic[nd])
    ...
    return G, list_of_Graphs_final, Gagr, edgeList ,nmap ,mapping,k,n#F

def plot_graph_dict(G,n,k,list_of_Graphs_final, Gagr,d1=0.8,d2=5.0,nodesize=1000,withlabels=True,edgelist=[],layout=True,b_alpha=0.5):  
    '''
    Plotting the synthetic graph after increasing the distance among layers by a parameter d1
    and dilating each layer by a parameter d1 
    '''
...
def plot_total_graph_with_weights(graph_dic,nodper_dic):

    G=nx.Graph()
    for k,v in graph_dic.items():
        for edge in v.edges(data=True):
            ed=edge[0]
            de=edge[1]
            weigt=edge[2]['weight']
            # attd=edge[2]
            ...
            if G.has_edge(ed,de):
                ...
                wei=0
            G.add_edge(ed,de,weight=wei+weigt)
        for nod in v.nodes(data=True):
            G.add_node(nod[0],attr_dict=nod[1])
    return G
